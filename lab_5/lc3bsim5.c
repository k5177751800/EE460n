...
void eval_micro_sequencer() {
    ...
    if (CURRENT_LATCHES.INT) {
        ...
        int j_bits = GetJ(CURRENT_LATCHES.MICROINSTRUCTION);
        int cond =
                (CURRENT_LATCHES.MICROINSTRUCTION[COND1] << 1) |
        ...
        switch (cond) {
            case 0:
                break;
            case 1:
                ms = (CURRENT_LATCHES.READY)<<1;
                break;
            case 2:
                ms = (CURRENT_LATCHES.BEN)<<2;
                break;
            case 3:
                ms = ((CURRENT_LATCHES.IR>>11)&0x1);
                break;
        }
        int next_state = j_bits | (ms);
    ...
    if (GetIRD(currentMicroinstruction)) {
        ...
        int jBits = (currentMicroinstruction[J5] << 5) |
                    ...
                    ((currentMicroinstruction[J2] || (CURRENT_LATCHES.BEN && currentMicroinstruction[COND1] && !currentMicroinstruction[COND0])) << 2) |
                    ((currentMicroinstruction[J1] || (CURRENT_LATCHES.READY && currentMicroinstruction[COND0] && !currentMicroinstruction[COND1])) << 1) |
                    ((currentMicroinstruction[J0] || (CURRENT_LATCHES.IR & 0x0800) >> 11 && currentMicroinstruction[COND1] && currentMicroinstruction[COND0]));
...
int Gate_MDR, Gate_ALU, Gate_MARMUX, Gate_SHF, Gate_PC, Gate_PSR, Gate_Vec, Gate_SP;
int tmp;
int vecProc = (0x2 <<1) + 0x0200;
int vecUnal = (0x3 << 1) +0x0200;
void eval_bus_drivers() {
    /*
     ...
     */
    int sReg = 0;
    if (CURRENT_LATCHES.MICROINSTRUCTION[SR1MUX1]) {
        sReg = 6;
    } else {
        sReg = (GetSR1MUX(CURRENT_LATCHES.MICROINSTRUCTION))?((CURRENT_LATCHES.IR & 0x01C0) >> 6):((CURRENT_LATCHES.IR>>9)&0x7);
    }
    ...
    int baseOffset = (CURRENT_LATCHES.IR & 0x3F);
    if (baseOffset & 0x20) {
        baseOffset |= 0xFFFFFFC0;
    }
...
//    offSet += baseOffset << shiftA;
    ...
    if (GetMARMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
        Gate_MARMUX = aluMath;
    } else if (GetMARMUX(CURRENT_LATCHES.MICROINSTRUCTION) == 0){
...
void drive_bus() {
    ...
    if(GetGATE_PC(CURRENT_LATCHES.MICROINSTRUCTION) == 1){
        ...
        BUS = Gate_SHF;
        return;
    }else if(CURRENT_LATCHES.MICROINSTRUCTION[GATE_SP]){
        BUS = Low16bits((CURRENT_LATCHES.MICROINSTRUCTION[SPMUX])?(CURRENT_LATCHES.REGS[6]+2):(CURRENT_LATCHES.REGS[6]-2));
        return;
    }else if(CURRENT_LATCHES.MICROINSTRUCTION[GATE_TMP]){
        BUS = Low16bits(CURRENT_LATCHES.TMP);
        return;
    }else if(CURRENT_LATCHES.MICROINSTRUCTION[GATE_PC2]){
...
void latch_datapath_values() {
     ...
     * after drive_bus.
     */
    if (CURRENT_LATCHES.EXC) return;

    CURRENT_LATCHES.priv = (CURRENT_LATCHES.PSR >> 15)&0x1;
...
//            NEXT_LATCHES.PC = Low16bits(BUS);
    ...
    if ((BUS >= 0 && BUS <= 0x2FFF) && CURRENT_LATCHES.priv && GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION)) {
        NEXT_LATCHES.EXC = 1;
        NEXT_LATCHES.EXCV = (0x2 << 1) + 0x0200;
    } else if (BUS % 2 != 0 && CURRENT_LATCHES.priv && GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) && GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION)) {
        NEXT_LATCHES.EXC = 1;
    ...
    if (CURRENT_LATCHES.MICROINSTRUCTION[LD_Vector]) {
        NEXT_LATCHES.Vector = (CURRENT_LATCHES.MICROINSTRUCTION[VECMUX1])?(0x200+(CURRENT_LATCHES.EXCV<<1)):(0x200+(CURRENT_LATCHES.INTV<<1));
    }
    //
    if (GetLD_REG(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
        ...
        switch (GetDRMUX(CURRENT_LATCHES.MICROINSTRUCTION)) {
            case 0:
                reg_num = (CURRENT_LATCHES.IR >> 9) & 0x07;
                break;
            case 1:
                reg_num = 7;
                break;
        }
        if (CURRENT_LATCHES.MICROINSTRUCTION[DRMUX1]) {
            reg_num = 6;
        }
    ...
    if (GetLD_CC(CURRENT_LATCHES.MICROINSTRUCTION)) {
        ...
        NEXT_LATCHES.PSR = (CURRENT_LATCHES.priv<<15)||(NEXT_LATCHES.N<<2)||(NEXT_LATCHES.Z<<1)||(NEXT_LATCHES.P);
    } else {
    ...
    if (GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
        ...
        //NEXT_LATCHES.MAR = LOW16bits(CURRENT_LATCHES.PFN<<9)|(BUS&0x1FF); //VA-PA Trans
    }
    // if (CURRENT_LATCHES.MICROINSTRUCTION[LD_PFN]) {}
    // if (CURRENT_LATCHES.MICROINSTRUCTION[LD_oPSR]) {}
    ...
    if (GetLD_MDR(CURRENT_LATCHES.MICROINSTRUCTION)){
        ...
        if (!GetMIO_EN(CURRENT_LATCHES.MICROINSTRUCTION) && !GetDATA_SIZE(CURRENT_LATCHES.MICROINSTRUCTION)) {
            NEXT_LATCHES.MDR = Low16bits((BUS & 0xFF) << 8*(CURRENT_LATCHES.MAR&0x1));
    ...
    if (CURRENT_LATCHES.MICROINSTRUCTION[LD_TMP]) {
        NEXT_LATCHES.TMP = Low16bits(BUS);
    }
    if (GetLD_BEN(CURRENT_LATCHES.MICROINSTRUCTION) ==1){
        ...
        NEXT_LATCHES.BEN = (ir11 && NEXT_LATCHES.N) || (ir10 && NEXT_LATCHES.Z) || (ir9 && NEXT_LATCHES.P);
...
void chekcMemVio() {
    bool isUnalignedAccess = (BUS % 2 != 0);
    bool isProtectedAccess = (BUS >= 0x0000 && BUS <= 0x2FFF);
    bool isOutOfRangeAccess = (BUS >= 0x3000 && BUS <= 0xFDFF);
    if (CURRENT_LATCHES.STATE_NUMBER == 2 || CURRENT_LATCHES.STATE_NUMBER == 6 ||
        CURRENT_LATCHES.STATE_NUMBER == 7 || CURRENT_LATCHES.STATE_NUMBER == 3) {
        if (CURRENT_LATCHES.priv == 1) {
            if (isProtectedAccess) {
                exception(vecProc);
            } else if ((CURRENT_LATCHES.STATE_NUMBER == 6 || CURRENT_LATCHES.STATE_NUMBER == 7) && isUnalignedAccess) {
                exception(vecUnal);
            }
        }
    }
    if (CURRENT_LATCHES.STATE_NUMBER == 37 || CURRENT_LATCHES.STATE_NUMBER == 51) {
        if (isUnalignedAccess) {
            exception(vecUnal);
        } else if (isOutOfRangeAccess) {
            exception(vecProc);
        }
    }
}

// Helper function to trigger an exception
void exception(int excVector) {
    NEXT_LATCHES.EXC = 1;
    NEXT_LATCHES.EXCV = excVector;
}